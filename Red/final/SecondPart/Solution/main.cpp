...
void TestTime(){LOG_DURATION("TestTime")
	string sentence;
	for (int i = 0; i != 25; ++i){
			string word(10, char(i+60));
			sentence += word + " ";
	}
  
  const vector<string> docs(50000, sentence);
	for (int i = 0; i != 10; ++i){
			string word(10, char(i+50));
			if (i == 9){
				sentence += word;
			} else {
				sentence += word + " ";
 	...
 	const vector<string> queries(10, sentence);
	istringstream docs_input(Join('\n', docs));
  istringstream queries_input(Join('\n', queries));
