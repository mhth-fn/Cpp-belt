...
public:
  ...
  AirportCounter(TIterator begin, TIterator end){
		for (int i = 0; i != airport_count_v.size(); ++i){
			airport_count_v[i] = 0;
		}
		while (begin != end){
			++airport_count_v[static_cast<int>(*begin)];
			++begin;
		}
	}
  ...
  size_t Get(TAirport airport) const{
			return airport_count_v[static_cast<int>(airport)];
	}

  // добавить данный элемент
  void Insert(TAirport airport){
			++airport_count_v[static_cast<int>(airport)];
	}
  ...
  void EraseAll(TAirport airport){
		airport_count_v[static_cast<int>(airport)] = 0;
	}
  ...
  Items GetItems() const{
		Items res;
		int count = airport_count_v.size();
		
		size_t i = 0;
		for (const auto& x : airport_count_v){
			res[i] = {static_cast<TAirport>(i), x};
			++i;
		}
		return res; //NlogN
	}
...
private:
  // ???
	array<int, static_cast<int>(TAirport::Last_)> airport_count_v;
...
void TestManyConstructions() {
  ...
  uint64_t total = 0;
	{LOG_DURATION("100'000'000 Constructions")
