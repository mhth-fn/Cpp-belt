...
vector<Spending> LoadFromXml(istream& input) {
  ...
  for (size_t i = 0; i != root.Children().size(); ++i){
    const Node& tmp = root.Children()[i];
    const string& category =tmp.AttributeValue<string>("category");
    const int& value =tmp.AttributeValue<int>("amount");
    res.push_back({category, value});
  }
