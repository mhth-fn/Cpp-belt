...
size_t operator() (const Address& address) const{
  const size_t coef = 2'946'901;
	...
	return (coef * coef * shash(address.city)
			+ coef * shash(address.street) +ihash(address.building));
	}
 hash<string> shash;
 hash<int> ihash;
...
struct PersonHasher {
	size_t operator() (const Person& person) const{
  	const size_t coef = 2'946'901;
		return (coef*coef*coef*shash(person.name) + coef*coef*ihash(person.height) 
				+ coef * dhash(person.weight) + adrhash(person.address));
	}
	 hash<string> shash;
	 hash<int> ihash;
	 hash<double> dhash;
