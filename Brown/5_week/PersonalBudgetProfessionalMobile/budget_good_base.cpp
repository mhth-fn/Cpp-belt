...
public:
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    ...
    spend_.spend += other.spend_.spend;
  }

  MoneyState<double> Collapse(MoneyState<double> origin, IndexSegment segment) const {
    return {origin.add * tax_.ComputeFactor() + add_.delta * segment.length(),
    origin.spend + spend_.spend*segment.length()};
  }
...
private:
  ...
  public:
    ...
    Data ProcessFull(const NodeHolder& node) const {
      return node->data.add - node->data.spend;
    }
  ...
  public:
    ...
    void ProcessPartial(const NodeHolder& node, IndexSegment) const {
      node->data.add = (node->left ? node->left->data.add : Data()) + (node->right ? node->right->data.add : Data());
      node->data.spend = (node->left ? node->left->data.spend : Data()) + (node->right ? node->right->data.spend : Data());
    }
