...
void TestGetReversedParts(const vector<Domain> domains){
  auto domain = domains[4].GetReversedParts();
  ASSERT_EQUAL(*domain.begin(), "ru");
  ASSERT_EQUAL(*(++domain.begin()), "z");
}

void TestIsSubdomain(const vector<Domain>& domains){
  ASSERT(!IsSubdomain(domains[0], domains[2]));
  ASSERT(IsSubdomain(domains[2], domains[0]));
  ASSERT(IsSubdomain(domains[1], domains[1]));
}

void TestIsSubOrSuperDomain(const vector<Domain>& domains){
  ASSERT(IsSubOrSuperDomain(domains[0], domains[2]));
  ASSERT(IsSubOrSuperDomain(domains[2], domains[0]));
  ASSERT(IsSubOrSuperDomain(domains[1], domains[1]));
}
...
void TestDomainChecker(){
  ...
  auto domains = ReadDomains(in_stream);
  DomainChecker test_checker(begin(domains), end(domains));

  stringstream test_stream;
  test_stream << "1\n" 
  << "l.ya.ru\n";
  auto test_domains = ReadDomains(test_stream);

  ASSERT(test_checker.IsSubdomain(test_domains[0]));
}

void TestCheckDomains(const vector<Domain> banned_domains){
  stringstream in_stream;
  in_stream << "2\n" 
  << "ya.ru\n" 
  << "m.ya.ru\n";
  auto domains_to_check = ReadDomains(in_stream);
  auto res = CheckDomains(banned_domains, domains_to_check);
  
  for (const bool item : res){
    ASSERT(!item);
  }
}
...
void TestPrintCheckResults(){
  ...
  const vector<bool> test_input = {false, true, true, false};

  PrintCheckResults(test_input, out_stream);

  const string expect = "Bad\nGood\nGood\nBad\n";
  ASSERT_EQUAL(out_stream.str(), expect);
}

void TestReadDomains(const vector<Domain>& domains){
  auto domain =domains[0].GetParts();
  ASSERT_EQUAL(domains.size(), 6);
  ASSERT_EQUAL(domains[0].GetPartCount(), 2);
  ASSERT_EQUAL(*domain.begin(), "ya");
  ASSERT_EQUAL(*(++domain.begin()), "ru");
  }
...
void TestSimple() {
  ...
  TestIsSubdomain(domains);
  // При проверке того, является ли один домен поддоменом другого, параметры перепутаны местами.
  TestIsSubOrSuperDomain(domains);
