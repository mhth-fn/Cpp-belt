...
public:
    Budget(){
        budget_.resize(DAY_COUNT);
    }

    void Earn(const Date& from, const Date& to, int value){
        if (from == to){
            budget_[ComputeDaysDiff(START_DATE, from)].earned += value;
        } else{
            ...
            while (from_index != to_index){
                budget_[from_index++].earned += daily_value;
            }
        }
    }
    void Spend(const Date& from, const Date& to, int value){
        if (from == to){
            budget_[ComputeDaysDiff(START_DATE, from)].spend += value;
        } else{
            int from_index = ComputeDaysDiff(START_DATE, from);
            int to_index = ComputeDaysDiff(START_DATE, to) + 1;
            double daily_spend = (1.0 * value) / (to_index - from_index);
            while (from_index != to_index){
                budget_[from_index++].spend += daily_spend;
            }
        }
    }
    double ComputeIncome(const Date& from, const Date& to) const{
        if (from == to){
            size_t index = ComputeDaysDiff(START_DATE, from);
            return budget_[index].earned - budget_[index].spend;
        } else {
            int from_index = ComputeDaysDiff(START_DATE, from);
            int to_index = ComputeDaysDiff(START_DATE, to) + 1;
            return std::accumulate(budget_.begin() + from_index, budget_.begin() + to_index, 0.0, [](double sum, const MoneyState ms){
                return sum + ms.earned;
            }) - std::accumulate(budget_.begin() + from_index, budget_.begin() + to_index, 0.0, [](double sum, const MoneyState ms){
                return sum + ms.spend;
            }) ;
        } 
    }
    ...
    void PayTax(const Date& from, const Date& to, int percentage){
        double per = (100.0 - percentage) / 100.0;
        if (from == to){
            budget_[ComputeDaysDiff(START_DATE, from)].earned *= per;
        } else{
            int from_index = ComputeDaysDiff(START_DATE, from);
            int to_index = ComputeDaysDiff(START_DATE, to) + 1;
            while (from_index != to_index){
                budget_[from_index++].earned *= per;
            }
        }
    }
...
int main(){
    ...
    for (int i = 0; i != query; ++i){
        ...
        if (command == "Earn"){
            ...
            int value;
            cin >> value;
            personal.Spend(Date::FromString(date_from), 
                                Date::FromString(date_to), value);
