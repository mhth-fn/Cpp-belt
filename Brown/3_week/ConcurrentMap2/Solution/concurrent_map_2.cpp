...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count) : m(bucket_count){
    busket.resize(bucket_count);
  }

  WriteAccess operator[](const K& key){
   int current_thread = hasher(key) % busket.size();
   return {lock_guard(m[current_thread]), busket[current_thread][key]};
  }
  ReadAccess At(const K& key) const{
   int current_thread = hasher(key) % busket.size();
   return {lock_guard(m[current_thread]), busket[current_thread].at(key)};
  }

  bool Has(const K& key) const{
   int current_thread = hasher(key) % busket.size();
   return busket[current_thread].count(key) == 1;
  }

  MapType BuildOrdinaryMap() const{
    MapType res;
    for (size_t i = 0; i != busket.size(); ++i){
      lock_guard<mutex> g{m[i]};
      res.insert(busket[i].begin(), busket[i].end());
    }
    return res;
  }
...
private:
  Hash hasher;
  mutable vector<mutex> m;
  vector<MapType> busket;
};
