...
public:
  ...
  class Proxy{
    public:
    Proxy(size_t degree, Polynomial& pol) : degree_(degree), poly(pol){}

    void operator=(const T& lhs){
      if (poly.coeffs_.size() <= degree_ && lhs != 0){
        poly.coeffs_.resize(degree_ + 1);
      }
      poly.coeffs_[degree_] = lhs;
      poly.Shrink();
    }

    operator T() const{
      return as_const(poly)[degree_];
    }
    private:
      Polynomial& poly;
      size_t degree_;
